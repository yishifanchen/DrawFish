...
public class PaletteLU : MonoBehaviour {
    DrawLU m_draw;
    ...
    void Start()
    {
        ...
        m_selectRect2 = new Rect(((float)Screen.width / 1920 * 100), ((float)Screen.height / 1080 * 38), m_paletteTex2D.width, m_paletteTex2D.height);
        m_selectRect1 = new Rect(((float)Screen.width / 1920 * 100) + m_paletteTex2D.width + 5, ((float)Screen.height / 1080 * 38), m_swatchesWidth, m_swatchesHeight);

        m_selectRect2fan = new Rect(((float)Screen.width / 1920 * 860) - m_paletteTex2D.width, ((float)Screen.height / 1080 * 502) - m_paletteTex2D.height, m_paletteTex2D.width, m_paletteTex2D.height);
        m_selectRect1fan = new Rect(((float)Screen.width / 1920 * 860) - m_paletteTex2D.width - m_swatchesWidth - 5, ((float)Screen.height / 1080 * 502) - m_swatchesHeight, m_swatchesWidth, m_swatchesHeight);
        f_SetSwatchesPanel();
    ...
    IEnumerator f_DelayDraw()
    {
        ...
        GetComponent<PaletteLU>().enabled = false;
    ...
    void MouseInput()//鼠标输入
    {
        ...
        if (!m_selectRect1fan.Contains(m_mousePosition) && !m_selectRect2fan.Contains(m_mousePosition) && Config.m_rectLU.Contains(Input.mousePosition))
        {
            if (Input.GetMouseButtonDown(0))
            {
                StartCoroutine(f_DelayDraw());
    ...
    void TouchInput()//触控输入
    {
        for (int i = 0; i < Input.touchCount; i++)
        {
            ...
            if (!m_selectRect1fan.Contains(m_mousePosition) && !m_selectRect2fan.Contains(m_mousePosition) && Config.m_rectLU.Contains(Input.GetTouch(i).position))
            {
                if (Input.GetTouch(i).phase == TouchPhase.Began)
                {
                    StartCoroutine(f_DelayDraw());
