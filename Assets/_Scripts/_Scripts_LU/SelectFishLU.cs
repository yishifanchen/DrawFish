...
public class SelectFishLU : MonoBehaviour {
    GameControllerLU m_gameController;
	...
	void Start()
    {
        m_gameController = GameObject.Find("GameController_LU").GetComponent<GameControllerLU>();
    ...
    void MouseInput()
    {
        if (Input.GetMouseButtonDown(0) && Config.m_rectLU.Contains(Input.mousePosition))
        {
            ...
            if (Physics.Raycast(m_ray, out m_hitObject))
            {
                ...
                this.gameObject.SendMessage("f_JudgeFishTypeLU", m_fishMark);
