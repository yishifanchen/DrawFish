...
public class GameControllerRD : MonoBehaviour {
    ...
    void Start()
    {
        ...
        m_palette = GetComponent<PaletteRD>();
        ...
        if (m_image_Introduce.gameObject.activeSelf)
        {
            switch (Config.m_curInputType)
            {
                case Config.InputType.MouseInput:
                    if (Input.GetMouseButtonDown(0) && Config.m_rectRD.Contains(Input.mousePosition))
                    {
                ...
                case Config.InputType.TouchInput:
                    ...
                    {
                        if (Input.GetTouch(i).phase == TouchPhase.Began && Config.m_rectRD.Contains(Input.GetTouch(i).position))
                        {
                            m_image_Draw.gameObject.SetActive(true);
                            m_image_Introduce.gameObject.SetActive(false);
                        }
    ...
    public void f_GenerateTex2D()//生成2D贴图
    {
        ...
        m_draw.f_FishLineFrameMask(m_generateTex2D);
    ...
    public void f_ButtonShow()//展示按钮
    {
        ...
        m_curFish=Instantiate(m_fishType[m_draw.m_fishTemplateCount], Config.m_showFishPos_RD, Quaternion.Euler(m_fishType[m_draw.m_fishTemplateCount].transform.rotation.eulerAngles.x, m_fishType[m_draw.m_fishTemplateCount].transform.rotation.eulerAngles.y-90, m_fishType[m_draw.m_fishTemplateCount].transform.rotation.eulerAngles.z))as GameObject;
