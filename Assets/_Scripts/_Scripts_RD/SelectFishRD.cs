...
public class SelectFishRD : MonoBehaviour {
    GameControllerRD m_gameController;
	...
	void Start()
    {
        m_gameController = GameObject.Find("GameController_RD").GetComponent<GameControllerRD>();
    ...
    void MouseInput()
    {
        if (Input.GetMouseButtonDown(0) && Config.m_rectRD.Contains(Input.mousePosition))
        {
            ...
            if (Physics.Raycast(m_ray, out m_hitObject))
            {
                ...
                this.gameObject.SendMessage("f_JudgeFishTypeRD", m_fishMark);
