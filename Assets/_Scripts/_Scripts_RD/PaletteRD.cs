...
public class PaletteRD : MonoBehaviour {
    DrawRD m_draw;
    ...
    void Start()
    {
        ...
        m_selectRect2 = new Rect(((float)Screen.width / 1920 * 1060), ((float)Screen.height / 1080 * 578), m_paletteTex2D.width, m_paletteTex2D.height);
        m_selectRect1 = new Rect(((float)Screen.width / 1920 * 1060) + m_paletteTex2D.width + 5, ((float)Screen.height / 1080 * 578), m_swatchesWidth, m_swatchesHeight);
        f_SetSwatchesPanel();
        f_SetColorPanel(Color.red);
        m_offestY = (int)((float)Screen.height / 1080 * 578) + m_swatchesHeight;
    }
    void Update()
    {
    ...
    void OnGUI()
    {
        if (m_isShowPalette)
        {
            GUI.DrawTexture(m_selectRect2, m_paletteTex2D);
            GUI.DrawTexture(m_selectRect1, m_swatchesTex2D);
            GUI.depth = 0;
        }
    }
    ...
    IEnumerator f_DelayDraw()
    {
        ...
        GetComponent<PaletteRD>().enabled = false;
    ...
    void MouseInput()//鼠标输入
    {
        ...
        if (m_selectRect1.Contains(m_mousePosition))
        {
            if (Input.GetMouseButton(0))
            {
                Color m_selectColor = m_swatchesTex2D.GetPixel((int)(m_mousePosition.x - m_selectRect1.x), m_offestY - (int)m_mousePosition.y);
                f_SetColorPanel(m_selectColor);
            }
        }
        if (m_selectRect2.Contains(m_mousePosition))
        {
            if (Input.GetMouseButtonDown(0))
            {
                m_draw.m_curColor = m_paletteTex2D.GetPixel((int)(m_mousePosition.x - m_selectRect2.x), m_offestY - (int)m_mousePosition.y);
                StartCoroutine(f_DelayDraw());
            }
        }
        if (!m_selectRect1.Contains(m_mousePosition) && !m_selectRect2.Contains(m_mousePosition) && Config.m_rectRD.Contains(Input.mousePosition))
        {
            if (Input.GetMouseButtonDown(0))
            {
                StartCoroutine(f_DelayDraw());
    ...
    void TouchInput()//触控输入
    {
        for (int i = 0; i < Input.touchCount; i++)
        {
            ...
            if (!m_selectRect1.Contains(m_mousePosition) && !m_selectRect2.Contains(m_mousePosition) && Config.m_rectRD.Contains(Input.GetTouch(i).position))
            {
                if (Input.GetTouch(i).phase == TouchPhase.Began)
                {
                    StartCoroutine(f_DelayDraw());
