...
public class SelectFish : MonoBehaviour {
	...
	void Start()
    {
        m_gameController = GameObject.Find("GameController_LD").GetComponent<GameController>();
    ...
    void MouseInput()
    {
        if (Input.GetMouseButtonDown(0) && Config.m_rectLD.Contains(Input.mousePosition))//鼠标操作
        {
            ...
            if (Physics.Raycast(m_ray, out m_hitObject))
            {
                ...
                this.gameObject.SendMessage("f_JudgeFishType", m_fishMark);
