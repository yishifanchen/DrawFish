...
public class Draw : MonoBehaviour {
    ...
    void Start()
    {
        ...
        m_left = ((float)Screen.width/1920*278);//(float)(Screen.width / i);
        m_down = ((float)Screen.height/1080*114);//(float)(Screen.height / j);
            ...
            switch (Config.m_curInputType)
            {
                    ...
                    switch (m_curTool)
                    {
                        case ToolType.Pencil:
                            GUI.DrawTexture(new Rect(
                                (int)Input.mousePosition.x - (m_curToolPic2D.width / 2 * (int)m_sliderPencil.value / (int)m_sliderPencil.maxValue),
                ...
                case Config.InputType.TouchInput:
                    switch (m_curTool)
                    {
                        case ToolType.Pencil:
                            GUI.DrawTexture(new Rect(
                                (int)m_touchPos.x - (m_curToolPic2D.width / 2 * (int)m_sliderPencil.value / (int)m_sliderPencil.maxValue),
    ...
    public void f_JudgeFishType(string str)//判断鱼底图的类型
    {
        switch (str)
        {
            case "天使鱼":
    ...
    public void f_FishLineFrameMask(Texture2D texture2d)//蒙板
    {
        int x = 0;
        Color[] m_Bg = m_fishTemplateFgPic2D[m_fishTemplateCount].GetPixels(0, 0, m_fishTemplateFgPic2D[m_fishTemplateCount].width, m_fishTemplateFgPic2D[m_fishTemplateCount].height);
        for (int j = 0; j < m_fishTemplateFgPic2D[m_fishTemplateCount].height; j++)
