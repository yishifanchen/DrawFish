...
public class GameController : MonoBehaviour {
        ...
        if (m_image_Introduce.gameObject.activeSelf)
        {
            switch (Config.m_curInputType)
            {
                case Config.InputType.MouseInput:
                    if (Input.GetMouseButtonDown(0)&& Config.m_rectLD.Contains(Input.mousePosition))
                    {
                ...
                case Config.InputType.TouchInput:
                    ...
                    {
                        if (Input.GetTouch(i).phase == TouchPhase.Began && Config.m_rectLD.Contains(Input.GetTouch(i).position))
                        {
                            m_image_Draw.gameObject.SetActive(true);
                            m_image_Introduce.gameObject.SetActive(false);
                        }
    ...
    public void f_ButtonShow()//展示按钮
    {
        ...
        m_curFish=Instantiate(m_fishType[m_draw.m_fishTemplateCount], Config.m_showFishPos_LD, Quaternion.Euler(m_fishType[m_draw.m_fishTemplateCount].transform.rotation.eulerAngles.x, m_fishType[m_draw.m_fishTemplateCount].transform.rotation.eulerAngles.y-90, m_fishType[m_draw.m_fishTemplateCount].transform.rotation.eulerAngles.z))as GameObject;
