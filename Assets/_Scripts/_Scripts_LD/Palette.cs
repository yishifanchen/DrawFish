...
public class Palette : MonoBehaviour {
    ...
    void Start()
    {
        ...
        m_selectRect2 = new Rect(((float)Screen.width / 1920 * 100), ((float)Screen.height / 1080 * 578), m_paletteTex2D.width, m_paletteTex2D.height);
        m_selectRect1 = new Rect(((float)Screen.width / 1920 * 100)+m_paletteTex2D.width + 5, ((float)Screen.height / 1080 * 578), m_swatchesWidth, m_swatchesHeight);
        f_SetSwatchesPanel();
    ...
    IEnumerator f_DelayDraw()
    {
        ...
        GetComponent<Palette>().enabled = false;
    ...
    void MouseInput()//鼠标输入
    {
        ...
        if (!m_selectRect1.Contains(m_mousePosition) && !m_selectRect2.Contains(m_mousePosition) && Config.m_rectLD.Contains(Input.mousePosition))
        {
            if (Input.GetMouseButtonDown(0))
            {
                StartCoroutine(f_DelayDraw());
    ...
    void TouchInput()//触控输入
    {
        for (int i = 0; i < Input.touchCount; i++)
        {
            ...
            if (!m_selectRect1.Contains(m_mousePosition) && !m_selectRect2.Contains(m_mousePosition) && Config.m_rectLD.Contains(Input.GetTouch(i).position))
            {
                if (Input.GetTouch(i).phase == TouchPhase.Began)
                {
                    StartCoroutine(f_DelayDraw());
